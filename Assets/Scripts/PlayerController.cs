...
public class PlayerController : MonoBehaviour
{
    ...
    public float shootAnimationTiming;
    bool isRanged;
    #endregion
    ...
    void Update()
    {
        ...
        if ((Input.GetKeyDown(KeyCode.JoystickButton6) || Input.GetKeyDown(KeyCode.L)) && rangeTimer <= 0)
            Shoot();
        else
    ...
    IEnumerator ShootRoutine()
    {
        ...
        yield return new WaitForSeconds(shootAnimationTiming);
    ...
    public void Respawn()
    {
        ...
        for (int i = checkpoints.Length - 1; i >= 0; i--)
        {
            ...
            {
                //Debug.Log("True: " + i);
                playerRB.position = checkpoints[i].GetComponent<Transform>().position + Vector3.right;
    ...
    public void CallKnockback(bool knockToRight)
    {
        StartCoroutine(Knockback(knockToRight));
    }
    private IEnumerator Knockback(bool knockToRight)
    {
        float timer = 0;
        ...
        while (knockDur > timer)
        {
            ...
            //playerRB.AddForce(new Vector3(knockbackDir.x * -100, knockbackDir.y + knockbackPwr));
            if (knockToRight)
            {
                playerRB.velocity = new Vector2(knockbackPwr, 0);
            }
            else
            {
                playerRB.velocity = new Vector2(-knockbackPwr, 0);
